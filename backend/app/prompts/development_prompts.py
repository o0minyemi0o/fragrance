...
def get_development_system_prompt(ingredient_list: str = "", ingredient_count: int = 0) -> str:
    ...
    """
    # Include ingredient info
    ingredient_info = ""
    if ingredient_count > 0:
        ingredient_info = f"""
**Available DB Ingredients ({ingredient_count} total):**
{ingredient_list}

**IMPORTANT**: Prioritize using ingredients from the above list when creating formulas.
"""
...
1. **Ingredient Selection Strategy**:
   - PRIORITIZE using ingredients from the Available DB Ingredients list below
...
2. **Material Suggestions (Messages 4-6):**
   - Suggest a COMPLETE palette of above 20 ingredients
...
3. **Formula Development (Messages 7-10):**
   - Develop final formula with above 20 ingredients total
